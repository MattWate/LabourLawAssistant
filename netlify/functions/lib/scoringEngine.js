...
function scoreByTrack(track, facts, breakdown) {
  ...
  if (track === 'UD-MISCONDUCT') {
    ...
    if (facts.proc_chair === false || clean(facts.proc_chair).includes('no') || hasAny(story, ['chair was my manager', 'biased chair'])) { procedural += 3; add(breakdown, 'procedural', 3, 'Chairperson independence appears defective', 'Schedule 8 Item 4'); }
  }
  ...
  if (track === 'UD-INCAPACITY') {
    if (hasAny(story, ['temporary', 'recover', 'medical certificate', 'sick note'])) { substantive += 3; add(breakdown, 'substantive', 3, 'Temporary incapacity or medical evidence supports accommodation', 'Schedule 8 Items 10 and 11'); }
    if (facts.proc_consultation === false || hasAny(story, ['no consultation', 'brief meeting', 'only one meeting'])) { procedural += 4; add(breakdown, 'procedural', 4, 'Inadequate consultation about the incapacity', 'Schedule 8 Items 10 and 11'); }
    if (hasAny(story, ['no medical review', 'ignored medical'])) { procedural += 3; add(breakdown, 'procedural', 3, 'Employer may not have reviewed medical evidence properly', 'Schedule 8 Items 10 and 11'); }
    if (hasAny(story, ['no alternative', 'no accommodation', 'refused accommodation', 'adapted duties'])) { procedural += 3; substantive += 2; add(breakdown, 'procedural', 3, 'Alternatives to dismissal may not have been explored', 'Schedule 8 Items 10 and 11'); add(breakdown, 'substantive', 2, 'Failure to consider accommodation strengthens the employee position', 'EEA s15'); }
    substantive += tenurePoint(facts, breakdown);
  }
  ...
  if (track === 'UD-RETRENCHMENT') {
    ...
    if (facts.proc_consultation === false || hasAny(story, ['no consultation', 'brief consultation'])) { procedural += 3; add(breakdown, 'procedural', 3, 'Consultation appears inadequate', 'LRA s189'); }
  ...
  if (track === 'PDA') {
    if (facts.proc_notice === false || hasAny(story, ['less than 48 hours', 'same day notice', 'no charge sheet'])) { procedural += 3; add(breakdown, 'procedural', 3, 'Charge sheet or notice period appears defective', 'Schedule 8 Item 4'); }
    if (facts.proc_rep === false || hasAny(story, ['no representative', 'not allowed representation'])) { procedural += 2; add(breakdown, 'procedural', 2, 'Representation may have been refused', 'Schedule 8 Item 4'); }
  ...
  if (track === 'ANC') {
    ...
    if (hasAny(story, ['raised internally', 'grievance', 'emailed hr', 'appealed'])) { procedural += 2; add(breakdown, 'procedural', 2, 'Internal escalation attempt recorded', 'Internal process / LRA ULP threshold'); }
  }
